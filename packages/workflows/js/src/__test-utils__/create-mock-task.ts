...
export interface CreateMockTaskOptions<TRunnerInput = any> {
  name: Task<TRunnerInput>['name'];
}

export type MockPromiseTask<
  TRunnerInput extends Record<PropertyKey, any> = any,
  TRunnerOutput extends Record<PropertyKey, any> = any,
> = Task<TRunnerInput, TRunnerOutput>;

export type MockObservableTask<
  TRunnerInput extends Record<PropertyKey, any> = any,
  TRunnerOutput extends Record<PropertyKey, any> = any,
> = Task<TRunnerInput, TRunnerOutput>;
...
export function createMockPromiseTask<
  TRunnerInput extends Record<PropertyKey, any> = any,
  TRunnerOutput extends Record<PropertyKey, any> = any,
...
export function createMockObservableTask<
  TRunnerInput extends Record<PropertyKey, any> = any,
  TRunnerOutput extends Record<PropertyKey, any> = any,
>(options: CreateMockTaskOptions<TRunnerInput>) {
  const { name } = options;
    ...
    runner: ({ sendUpdate }) => {
      return new Promise<void>((resolve) => {
        const tick = (updateCount: number) => {
          ...
          if (isLastTick) {
            sendUpdate({ completePercent: 1, message: 'Done' });
            resolve();
          } else {
